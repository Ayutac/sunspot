...
public class ExtractMenu extends AbstractContainerMenu {
    ...
    public ExtractMenu(final int containerId, final @NotNull Inventory inventory) {
        this(containerId, inventory, new SimpleContainer(SLOT_COUNT), new SimpleContainerData(1));
    }

    public ExtractMenu(final int containerId, final @NotNull Inventory playerInventory, final @NotNull Container inventory, final @NotNull ContainerData data) {
        super(SPMenuTypeRegistry.EXTRACT.get(), containerId);
        container = inventory;
        containerData = data;
